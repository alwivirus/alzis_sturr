...
<div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 28px; max-width: 960px;">
    ...
    <div style="margin-bottom: 24px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px;">
            ...
            @endphp
            <button type="button" id="tab-btn-game" class="partner-type-tab {{ $currentActiveType === 'game_account' ? 'active' : '' }}" onclick="switchPartnerProductType('game_account')">
            ...
            <button type="button" id="tab-btn-app" class="partner-type-tab {{ $currentActiveType === 'app_premium' ? 'active' : '' }}" onclick="switchPartnerProductType('app_premium')">
                <span style="font-size: 1.2rem;">📱</span>
                <div style="text-align: left;">
            ...
            <button type="button" id="tab-btn-service" class="partner-type-tab {{ $currentActiveType === 'fast_tournament' ? 'active' : '' }}" onclick="switchPartnerProductType('fast_tournament')">
                <span style="font-size: 1.2rem;">🏆</span>
                <div style="text-align: left;">
    ...
    <form action="{{ route('partner.accounts.update', $account->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <input type="hidden" name="product_type" id="product_type_input" value="{{ $currentActiveType }}">
