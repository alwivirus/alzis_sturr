...
@section('title', 'Edit Produk & Akun #' . $account->code . ' - ALZIS STORE Admin')
...
<div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 28px; max-width: 1000px;">
    ...
    <form action="{{ route('admin.accounts.update', $account->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 14px; margin-bottom: 16px;">
            ...
            <div class="form-group">
                <label class="form-label" id="new-category-label">Atau Ubah / Tulis Kategori Baru</label>
                <input type="text" name="new_game_name" value="{{ old('new_game_name') }}" class="input-control" placeholder="Misal: Canva, CapCut, Steam, Roblox...">
                <span class="form-helper" style="font-size: 0.72rem; color: #fbbf24;">Isi jika ingin memindahkan ke kategori baru.</span>
            </div>

            <div class="form-group">
                <label class="form-label">Kode SKU / Kode Produk <span style="color: var(--danger);">*</span></label>
                <input type="text" name="code" value="{{ old('code', $account->code) }}" class="input-control" required style="font-family: monospace; font-weight: 800;">
        ...
        <div class="form-group" style="margin-bottom: 18px;">
            ...
            <input type="text" name="title" id="product_title_input" value="{{ old('title', $account->title) }}" class="input-control" required>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px; margin-bottom: 18px;">
            <div class="form-group">
                <label class="form-label">Harga Asli / Normal (Rp) <span style="color: var(--danger);">*</span></label>
            ...
            <div class="form-group">
                ...
                <input type="number" name="discount_price" value="{{ old('discount_price', $account->discount_price ? (int)$account->discount_price : '') }}" class="input-control" placeholder="Biarkan kosong jika tanpa diskon">
        ...
        <!-- 3. Seller / Partner WhatsApp Contact -->
        <div class="form-group" style="margin-bottom: 20px; background: rgba(255,255,255,0.02); border: 1px solid var(--border); border-radius: 10px; padding: 14px 16px;">
            <label class="form-label" style="color: #fbbf24; font-weight: 800; display: flex; align-items: center; gap: 6px; margin-bottom: 6px;">
                ...
                <span>Nomor WhatsApp Penjual / Pemilik Akun (Opsional)</span>
            </label>
            <input type="text" name="partner_phone" value="{{ old('partner_phone', ($account->user && $account->user->phone) ? $account->user->phone : '') }}" class="input-control" placeholder="Contoh: 081234567890 / 6281234567890">
            <span class="form-helper" style="font-size: 0.72rem; color: var(--text-dim); margin-top: 4px;">Isi jika produk/akun ini milik mitra atau penjual tertentu (otomatis tampilkan tombol Chat Penjual di katalog).</span>
        </div>

        <!-- 5A. KHUSUS PRODUK APLIKASI (Canva, CapCut, Spotify, Alight Motion) -->
        <div id="section-app-details" style="display: none; background: rgba(245, 158, 11, 0.05); border: 1px solid rgba(245, 158, 11, 0.25); border-radius: 12px; padding: 18px; margin-bottom: 20px;">
            <div style="font-weight: 800; font-size: 0.95rem; color: #fbbf24; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
                ...
                <span>Detail Paket Aplikasi Digital (Canva / CapCut / Spotify)</span>
            </div>

            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 14px;">
                <div class="form-group">
                    <label class="form-label">Jumlah Stok Ready (Akun)</label>
                    <input type="number" name="stock_qty" value="{{ old('stock_qty', $account->stock_qty ?: 1) }}" min="1" class="input-control" placeholder="Contoh: 10">
                    <span class="form-helper">Berapa akun ready untuk dijual</span>
                </div>

                <div class="form-group">
                    <label class="form-label">Durasi / Masa Aktif</label>
                    <input type="number" name="duration_value" value="{{ old('duration_value', $account->duration_value ?: 1) }}" min="1" class="input-control" placeholder="Contoh: 1, 3, 12">
                ...
                <div class="form-group">
                    ...
                    <select name="account_variant" class="input-control">
                        ...
                        <option value="Akun Baru (Fresh)" {{ old('account_variant', $account->account_variant) == 'Akun Baru (Fresh)' ? 'selected' : '' }}>Akun Fresh (Dari Penjual)</option>
                    </select>
                </div>
            </div>
        </div>

        <!-- 5B. KHUSUS FAST TOURNAMENT (Slot Turnamen FT) -->
        <div id="section-ft-details" style="display: none; background: rgba(245, 158, 11, 0.05); border: 1px solid rgba(245, 158, 11, 0.25); border-radius: 12px; padding: 18px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px;">
                <div class="form-group">
                    ...
                    <input type="number" name="ft_stock_qty" value="{{ old('stock_qty', $account->stock_qty ?: 16) }}" min="1" class="input-control" placeholder="Contoh: 16, 32 Slot" onchange="document.querySelector('input[name=stock_qty]').value = this.value;">
                </div>

                <div class="form-group">
                    <label class="form-label">Format / Ketentuan Turnamen</label>
                    <input type="text" name="ft_login_bind" value="{{ old('login_bind', $account->login_bind) }}" class="input-control" placeholder="Contoh: Grup WA Peserta" onchange="document.getElementById('bindInput').value = this.value;">
                </div>
            </div>
        </div>

        <!-- 5C. KHUSUS AKUN GAME (Bind & Server & Game Specs) -->
        <div id="section-game-details">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 16px; margin-bottom: 18px;">
                <div class="form-group">
                    ...
                    <input type="text" id="bindInput" name="login_bind" value="{{ old('login_bind', $account->login_bind) }}" class="input-control" placeholder="Contoh: Moonton Sepaket (All Unbind)">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="serverInput" name="server" value="{{ old('server', $account->server) }}" class="input-control" placeholder="Contoh: Indonesia, Asia, Global">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 14px; margin-bottom: 18px;">
                <div class="form-group">
                    ...
                    <input type="text" name="rank_tier" value="{{ old('rank_tier', $account->rank_tier) }}" class="input-control" placeholder="Contoh: Mythical Glory">
                </div>

                <div class="form-group">
                    <label class="form-label">Level Akun</label>
                    <input type="text" name="winrate" value="{{ old('winrate', $account->winrate) }}" class="input-control" placeholder="Contoh: Level 66 / Lv 100">
                </div>

                <div class="form-group">
                    <label class="form-label">Total Skin</label>
                    <input type="number" name="skin_count" value="{{ old('skin_count', $account->skin_count) }}" class="input-control" placeholder="Contoh: 280">
                </div>

                <div class="form-group">
                    <label class="form-label">Total Hero</label>
                    <input type="number" name="hero_count" value="{{ old('hero_count', $account->hero_count) }}" class="input-control" placeholder="Contoh: 124">
                </div>
            </div>
        </div>

        <!-- 6. Deskripsi & Rincian Produk -->
        <h3 class="font-heading" style="font-size: 1.15rem; color: var(--primary); margin: 24px 0 16px; border-bottom: 1px solid var(--border); padding-bottom: 8px; font-weight: 800; display: flex; align-items: center; gap: 8px;">
        ...
        <div class="form-group" style="margin-bottom: 14px;">
            ...
            <input type="text" name="short_description" value="{{ old('short_description', $account->short_description) }}" class="input-control" placeholder="Contoh: Garansi resmi full, proses cepat 5 menit langsung aktif.">
        </div>

        <div class="form-group" style="margin-bottom: 20px;">
            <label class="form-label">Deskripsi Lengkap & Rincian Fitur</label>
            <textarea name="full_specs" id="full_specs_textarea" rows="5" class="input-control">{{ old('full_specs', $account->full_specs) }}</textarea>
        </div>

        <!-- 7. Foto Thumbnail & Screenshot -->
        <h3 class="font-heading" style="font-size: 1.15rem; color: var(--primary); margin: 24px 0 16px; border-bottom: 1px solid var(--border); padding-bottom: 8px; font-weight: 800; display: flex; align-items: center; gap: 8px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 16px; margin-bottom: 20px;">
            <div class="form-group">
                ...
                <input type="url" name="thumbnail_url" value="{{ old('thumbnail_url', str_starts_with($account->thumbnail ?? '', 'http') ? $account->thumbnail : '') }}" class="input-control" style="margin-top: 6px;" placeholder="https://contoh.com/foto.jpg">
            </div>
            ...
            <div class="form-group">
                ...
                <div style="margin-top: 12px;">
                    <span style="font-size: 0.75rem; color: var(--text-muted); display: block; margin-bottom: 6px; font-weight: 700;">Foto Galeri Saat Ini:</span>
                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                        @foreach($account->images as $img)
                        <div style="position: relative; width: 65px; height: 45px; border-radius: 6px; overflow: hidden; border: 1px solid var(--border);">
...
<script>
    function switchProductType(type) {
        ...
        if (type === 'game_account') {
            ...
            titleInput.placeholder = 'Contoh: Canva Pro 1 Tahun Private Email / Spotify Premium 3 Bulan';
